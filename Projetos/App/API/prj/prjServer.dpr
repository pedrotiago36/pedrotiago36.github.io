...
begin
          ...
          try
            ...
            try
              ...
              begin
                ...
                LObj.AddPair('is_admin',  TJSONNumber.Create(Ord(Qry.FieldByName('is_admin').AsBoolean)));
  ...
  THorse.Get('/professores',
    procedure(Req: THorseRequest; Res: THorseResponse)
    var Conn: TFDConnection; Qry: TFDQuery; LArr: TJSONArray; LObj: TJSONObject;
    begin
      try
        Conn := NewMySQLConnection;
        try
          ...
          try
            Qry.Connection := Conn;
            Qry.SQL.Text   :=
              'SELECT id, nome, serie, turma FROM tb_professores ' +
            ...
            try
              ...
              begin
                ...
                LObj.AddPair('id',    TJSONNumber.Create(Qry.FieldByName('id').AsInteger));
                LObj.AddPair('nome',  Qry.FieldByName('nome').AsString);
                LObj.AddPair('serie', Qry.FieldByName('serie').AsString);
                LObj.AddPair('turma', Qry.FieldByName('turma').AsString);
                LArr.Add(LObj);
                Qry.Next;
              end;
              Writeln(FormatDateTime('dd/mm/yyyy hh:nn:ss', Now) + ' GET /professores');
              Res.Status(200).Send<TJSONArray>(LArr);
            except LArr.Free; raise; end;
          finally Qry.Free; end;
        finally Conn.Free; end;
      except
        on E: Exception do Res.Status(500).Send('Erro: ' + E.Message);
      end;
    end
  );

  // POST /professores — cadastra novo professor
  // Body: {"nome":"...","serie":"...","turma":"..."}
  THorse.Post('/professores',
    procedure(Req: THorseRequest; Res: THorseResponse)
    var LRoot: TJSONObject; LNome, LSerie, LTurma: string;
        Conn: TFDConnection; Qry: TFDQuery; LID: Integer; LResp: TJSONObject;
    begin
      try
        LRoot := TJSONObject.ParseJSONValue(Req.Body) as TJSONObject;
        if not Assigned(LRoot) then begin Res.Status(400).Send('JSON invalido'); Exit; end;
        try
          LNome  := JStr(LRoot, 'nome',  '');
          LSerie := JStr(LRoot, 'serie', '');
          LTurma := JStr(LRoot, 'turma', '');
        finally LRoot.Free; end;
        if LNome.IsEmpty then begin Res.Status(400).Send('nome obrigatorio'); Exit; end;
        Conn := NewMySQLConnection;
        try
          Qry := TFDQuery.Create(nil);
          try
            Qry.Connection := Conn;
            Qry.SQL.Text :=
              'INSERT INTO tb_professores (nome, serie, turma) VALUES (:nome, :serie, :turma)';
            Qry.ParamByName('nome').AsString  := LNome;
            Qry.ParamByName('serie').AsString := LSerie;
            Qry.ParamByName('turma').AsString := LTurma;
            Qry.ExecSQL;
            LID := 0;
            Qry.SQL.Text := 'SELECT LAST_INSERT_ID() AS id'; Qry.Open;
            if not Qry.Eof then LID := Qry.FieldByName('id').AsInteger;
            LResp := TJSONObject.Create;
            try
              LResp.AddPair('sucesso', TJSONBool.Create(True));
              LResp.AddPair('id', TJSONNumber.Create(LID));
              Writeln(FormatDateTime('dd/mm/yyyy hh:nn:ss', Now) + ' POST /professores nome=' + LNome);
              Res.Status(201).Send<TJSONObject>(LResp);
            except LResp.Free; raise; end;
          finally Qry.Free; end;
        finally Conn.Free; end;
      except
        on E: Exception do Res.Status(500).Send('Erro: ' + E.Message);
  ...
  // Body: {"nome":"...","serie":"...","turma":"..."}
  THorse.Put('/professores/:id',
    procedure(Req: THorseRequest; Res: THorseResponse)
    var LID: Integer; LRoot: TJSONObject; LNome, LSerie, LTurma: string;
        Conn: TFDConnection; Qry: TFDQuery;
    begin
      try
        LID   := StrToIntDef(Req.Params['id'], 0);
        LRoot := TJSONObject.ParseJSONValue(Req.Body) as TJSONObject;
        ...
        try
          ...
          try
            Qry.Connection := Conn;
            Qry.SQL.Text :=
              'UPDATE tb_professores SET nome=:nome, serie=:serie, turma=:turma ' +
            ...
            Qry.ParamByName('id').AsInteger   := LID;
            Qry.ExecSQL;
            Writeln(FormatDateTime('dd/mm/yyyy hh:nn:ss', Now) + ' PUT /professores/' + IntToStr(LID));
            Res.Status(200).Send<TJSONObject>(
              TJSONObject.Create.AddPair('sucesso', TJSONBool.Create(True)));
          finally Qry.Free; end;
        finally Conn.Free; end;
  ...
  // DELETE /professores/:id — remove professor (soft delete)
  THorse.Delete('/professores/:id',
    procedure(Req: THorseRequest; Res: THorseResponse)
    var LID: Integer; Conn: TFDConnection; Qry: TFDQuery;
    begin
      try
        LID  := StrToIntDef(Req.Params['id'], 0);
        Conn := NewMySQLConnection;
        try
          ...
          try
            Qry.Connection := Conn;
            Qry.SQL.Text := 'UPDATE tb_professores SET ativo=0 WHERE id=:id';
            Qry.ParamByName('id').AsInteger := LID;
            Qry.ExecSQL;
            Writeln(FormatDateTime('dd/mm/yyyy hh:nn:ss', Now) + ' DELETE /professores/' + IntToStr(LID));
            Res.Status(200).Send<TJSONObject>(
              TJSONObject.Create.AddPair('sucesso', TJSONBool.Create(True)));
          finally Qry.Free; end;
        finally Conn.Free; end;
  ...
  // Body: {"aluno_matricula":"...","aluno_nome":"...","professor_id":N,"texto":"...","remetente":"aluno|professor"}
  THorse.Post('/mensagens',
    procedure(Req: THorseRequest; Res: THorseResponse)
    var LRoot: TJSONObject; LMatricula, LNome, LTexto, LRemetente: string;
        LProfID: Integer; Conn: TFDConnection; Qry: TFDQuery;
        LID: Integer; LResp: TJSONObject;
    begin
      try
        LRoot := TJSONObject.ParseJSONValue(Req.Body) as TJSONObject;
        if not Assigned(LRoot) then begin Res.Status(400).Send('JSON invalido'); Exit; end;
        try
          LMatricula := JStr(LRoot, 'aluno_matricula', '');
          LNome      := JStr(LRoot, 'aluno_nome',      '');
          LProfID    := JInt(LRoot, 'professor_id',    0);
          LTexto     := JStr(LRoot, 'texto',           '');
          LRemetente := JStr(LRoot, 'remetente',       'aluno');
        finally LRoot.Free; end;
        if LTexto.IsEmpty or (LProfID = 0) or LMatricula.IsEmpty then
        begin Res.Status(400).Send('Campos obrigatorios: aluno_matricula, professor_id, texto'); Exit; end;
        Conn := NewMySQLConnection;
        try
          Qry := TFDQuery.Create(nil);
          try
            ...
            Qry.SQL.Text :=
              ...
              '  (aluno_matricula, aluno_nome, professor_id, texto, remetente) ' +
              'VALUES (:matricula, :nome, :prof_id, :texto, :remetente)';
            Qry.ParamByName('matricula').AsString := LMatricula;
            Qry.ParamByName('nome').AsString      := LNome;
            Qry.ParamByName('prof_id').AsInteger  := LProfID;
            Qry.ParamByName('texto').AsString     := LTexto;
            Qry.ParamByName('remetente').AsString := LRemetente;
            Qry.ExecSQL;
            LID := 0;
            Qry.SQL.Text := 'SELECT LAST_INSERT_ID() AS id'; Qry.Open;
            if not Qry.Eof then LID := Qry.FieldByName('id').AsInteger;
            ...
            try
              ...
              LResp.AddPair('id', TJSONNumber.Create(LID));
              Writeln(FormatDateTime('dd/mm/yyyy hh:nn:ss', Now) +
                ' POST /mensagens prof=' + IntToStr(LProfID) + ' de=' + LRemetente);
              Res.Status(201).Send<TJSONObject>(LResp);
            except LResp.Free; raise; end;
          finally Qry.Free; end;
        finally Conn.Free; end;
  ...
  // GET /mensagens/professor/:professor_id
  // Lista conversas do professor: alunos distintos + qtd nao lidas
  THorse.Get('/mensagens/professor/:professor_id',
    procedure(Req: THorseRequest; Res: THorseResponse)
    var LProfID: Integer; Conn: TFDConnection; Qry: TFDQuery;
        LArr: TJSONArray; LObj: TJSONObject;
    begin
      try
        LProfID := StrToIntDef(Req.Params['professor_id'], 0);
        Conn    := NewMySQLConnection;
        try
          Qry := TFDQuery.Create(nil);
          try
            Qry.Connection := Conn;
            Qry.SQL.Text   :=
              'SELECT aluno_matricula, aluno_nome, ' +
              '  MAX(dt_envio) AS ultima_msg, ' +
              '  SUM(CASE WHEN remetente = ''aluno'' AND lida = 0 THEN 1 ELSE 0 END) AS nao_lidas ' +
              'FROM tb_mensagens ' +
              'WHERE professor_id = :prof_id ' +
              'GROUP BY aluno_matricula, aluno_nome ' +
            ...
            try
              ...
              begin
                LObj := TJSONObject.Create;
                LObj.AddPair('matricula',  Qry.FieldByName('aluno_matricula').AsString);
                LObj.AddPair('nome',       Qry.FieldByName('aluno_nome').AsString);
                LObj.AddPair('ultima_msg', Qry.FieldByName('ultima_msg').AsString);
                LObj.AddPair('nao_lidas',  TJSONNumber.Create(Qry.FieldByName('nao_lidas').AsInteger));
                LArr.Add(LObj);
                Qry.Next;
              end;
              Writeln(FormatDateTime('dd/mm/yyyy hh:nn:ss', Now) +
                ' GET /mensagens/professor/' + IntToStr(LProfID));
              Res.Status(200).Send<TJSONArray>(LArr);
            except LArr.Free; raise; end;
          finally Qry.Free; end;
        finally Conn.Free; end;
  ...
  // GET /mensagens/aluno/:matricula
  // Lista professores com quem o aluno ja conversou + qtd respostas nao lidas
  THorse.Get('/mensagens/aluno/:matricula',
    procedure(Req: THorseRequest; Res: THorseResponse)
    var LMatricula: string; Conn: TFDConnection; Qry: TFDQuery;
        LArr: TJSONArray; LObj: TJSONObject;
    begin
      try
        LMatricula := Req.Params['matricula'];
        Conn       := NewMySQLConnection;
        try
          Qry := TFDQuery.Create(nil);
          try
            Qry.Connection := Conn;
            Qry.SQL.Text   :=
              'SELECT m.professor_id, p.nome, p.serie, p.turma, ' +
              '  MAX(m.dt_envio) AS ultima_msg, ' +
              '  SUM(CASE WHEN m.remetente = ''professor'' AND m.lida = 0 THEN 1 ELSE 0 END) AS nao_lidas ' +
              'FROM tb_mensagens m ' +
              'INNER JOIN tb_professores p ON p.id = m.professor_id ' +
              'WHERE m.aluno_matricula = :matricula ' +
              'GROUP BY m.professor_id, p.nome, p.serie, p.turma ' +
              'ORDER BY ultima_msg DESC';
            Qry.ParamByName('matricula').AsString := LMatricula;
            Qry.Open;
            LArr := TJSONArray.Create;
            try
              ...
              begin
                LObj := TJSONObject.Create;
                LObj.AddPair('professor_id', TJSONNumber.Create(Qry.FieldByName('professor_id').AsInteger));
                LObj.AddPair('nome',         Qry.FieldByName('nome').AsString);
                LObj.AddPair('serie',        Qry.FieldByName('serie').AsString);
                LObj.AddPair('turma',        Qry.FieldByName('turma').AsString);
                LObj.AddPair('ultima_msg',   Qry.FieldByName('ultima_msg').AsString);
              ...
              Writeln(FormatDateTime('dd/mm/yyyy hh:nn:ss', Now) +
                ' GET /mensagens/aluno/' + LMatricula);
              Res.Status(200).Send<TJSONArray>(LArr);
            except LArr.Free; raise; end;
          finally Qry.Free; end;
        finally Conn.Free; end;
  ...
  // GET /mensagens/conversa/:professor_id/:matricula
  // Thread completo de uma conversa
  THorse.Get('/mensagens/conversa/:professor_id/:matricula',
    procedure(Req: THorseRequest; Res: THorseResponse)
    var LProfID: Integer; LMatricula: string;
        Conn: TFDConnection; Qry: TFDQuery; LArr: TJSONArray; LObj: TJSONObject;
    begin
      try
        LProfID    := StrToIntDef(Req.Params['professor_id'], 0);
        LMatricula := Req.Params['matricula'];
        ...
        try
          ...
          try
            Qry.Connection := Conn;
            Qry.SQL.Text   :=
              'SELECT id, texto, remetente, lida, ' +
              '  DATE_FORMAT(dt_envio, ''%d/%m %H:%i'') AS dt_fmt ' +
              'FROM tb_mensagens ' +
              'WHERE professor_id = :prof_id AND aluno_matricula = :matricula ' +
            ...
            try
              ...
              begin
                LObj := TJSONObject.Create;
                LObj.AddPair('id',        TJSONNumber.Create(Qry.FieldByName('id').AsInteger));
                LObj.AddPair('texto',     Qry.FieldByName('texto').AsString);
                LObj.AddPair('remetente', Qry.FieldByName('remetente').AsString);
                LObj.AddPair('lida',      TJSONNumber.Create(Ord(Qry.FieldByName('lida').AsBoolean)));
                LObj.AddPair('dt_envio',  Qry.FieldByName('dt_fmt').AsString);
                LArr.Add(LObj);
                Qry.Next;
              end;
              Writeln(FormatDateTime('dd/mm/yyyy hh:nn:ss', Now) +
                ' GET /mensagens/conversa/' + IntToStr(LProfID) + '/' + LMatricula);
              Res.Status(200).Send<TJSONArray>(LArr);
            except LArr.Free; raise; end;
          finally Qry.Free; end;
        finally Conn.Free; end;
  ...
  // PUT /mensagens/lida/professor/:professor_id/:matricula
  // Professor marca mensagens do aluno como lidas
  THorse.Put('/mensagens/lida/professor/:professor_id/:matricula',
    procedure(Req: THorseRequest; Res: THorseResponse)
    var LProfID: Integer; LMatricula: string; Conn: TFDConnection; Qry: TFDQuery;
    begin
      try
        LProfID    := StrToIntDef(Req.Params['professor_id'], 0);
        LMatricula := Req.Params['matricula'];
        ...
        try
          ...
          try
            Qry.Connection := Conn;
            Qry.SQL.Text   :=
              'UPDATE tb_mensagens SET lida = 1 ' +
              'WHERE professor_id = :prof_id AND aluno_matricula = :matricula ' +
              '  AND remetente = ''aluno''';
            Qry.ParamByName('prof_id').AsInteger  := LProfID;
            Qry.ParamByName('matricula').AsString := LMatricula;
  ...
  // PUT /mensagens/lida/aluno/:professor_id/:matricula
  // Aluno marca respostas do professor como lidas
  THorse.Put('/mensagens/lida/aluno/:professor_id/:matricula',
    procedure(Req: THorseRequest; Res: THorseResponse)
    var LProfID: Integer; LMatricula: string; Conn: TFDConnection; Qry: TFDQuery;
    begin
      try
